...
{
  // input simulation
  bool do_aprime{false},do_gun{false},do_dy{false},do_jpsi{false},do_cosmic(false);
  switch(isim){
  case 1: 
    do_aprime = true;
    break;
  ...
  rc->set_CharFlag("AlignmentMille", "$DIR_CMANTILL/macro/align_mille.txt"); // alignment file needed for EMCAL
  ...
  // these parameters affect how we reconstruct tracklets (a straight line segment in a single station) and St23 (combinations of pairs of tracklets from St2 and 3)
  // we usually try to minimize tracklets  (x,y,tx,ty) parameters
  if(is_displaced){
    rc->set_DoubleFlag("TX_MAX", 0.32); // maximum allowed x slope for a tracklet
    rc->set_DoubleFlag("TY_MAX", 0.2); // maximum allowed y slope for a tracklet 
    rc->set_DoubleFlag("X0_MAX", 500.0); // maximum allowed x position for a tracklet
    rc->set_DoubleFlag("Y0_MAX", 400.0); // maximum allowed y position for a tracklet
    rc->set_DoubleFlag("INVP_MAX", 0.5); // maximum inverse momentum (invp = 1/momentum)
  }
  rc->Print();
  ...
  SetupBeamline(g4Reco, do_collimator, target_coil_pos_z - 302.36); // collimator, targer and shielding between target and FMag
  if (do_target) {

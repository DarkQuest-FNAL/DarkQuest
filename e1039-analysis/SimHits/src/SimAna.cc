...
#define LogDebug(exp)                                                          \
  std::cout << "DEBUG: " << __FILE__ << ": " << __LINE__ << ": " << exp        \
            << std::endl
#define LogError(exp)                                                          \
  std::cout << "ERROR: " << __FILE__ << ": " << __LINE__ << ": " << exp        \
            << std::endl
#define LogWarning(exp)                                                        \
  std::cout << "WARNING: " << __FILE__ << ": " << __LINE__ << ": " << exp      \
            << std::endl
...
PHG4Hit *SimAna::FindG4HitAtStation(const int trk_id,
...
std::vector<PHG4Hit *>
SimAna::FindG4HitsAtStation(const int trk_id, const PHG4HitContainer *g4hc) {
...
void SimAna::checkKinematics(PHG4Particle *primary) {
  ...
  if (printPrimary) {
    ...
    std::cout << "primary shower z " << shower->get_z() << " shower id "
              << shower->get_id() << " pz " << primary->get_pz() << " e "
              << primary->get_e() << std::endl;
    std::cout << "edep emcal " << shower->get_edep(ECAL_volume) << std::endl;
    std::cout << " h1t " << shower->get_edep(h1t_volume) << " h1b "
              << shower->get_edep(h1b_volume) << " h1r "
              << shower->get_edep(h1r_volume) << " h1l "
              << shower->get_edep(h1l_volume);
    std::cout << " h2t " << shower->get_edep(h2t_volume) << " h2b "
              << shower->get_edep(h2b_volume) << " h2r "
              << shower->get_edep(h2r_volume) << " h2l "
              << shower->get_edep(h2l_volume);
    std::cout << " h3t " << shower->get_edep(h3t_volume) << " h3b "
              << shower->get_edep(h3b_volume) << std::endl;
    std::cout << " d0x " << shower->get_edep(d0x_volume) << " d1x "
              << shower->get_edep(d1x_volume);
    std::cout << " d2xp " << shower->get_edep(d2xp_volume) << " d3pxp "
              << shower->get_edep(d3pxp_volume) << " d3mxp "
              << shower->get_edep(d3mxp_volume) << std::endl;
    ...
    std::cout << "Primary " << primary->get_pid() << " vx " << gvx << " vy "
              << gvy << " vz " << gvz << std::endl;
    std::cout << " E " << primary->get_e() << std::endl;
    std::cout << " px " << primary->get_px() << " py " << primary->get_py()
              << " pz " << primary->get_pz() << std::endl;

    bool posel = false;
    if (primary->get_pid() == -11)
      posel = true;
    ...
    // dpx = dpt px/pz
    if (st1hit) {
      std::cout << "st1 x " << st1hit->get_x(0) << " y " << st1hit->get_y(0)
                << " z " << st1hit->get_z(0) << std::endl;
      std::cout << "    px " << st1hit->get_px(0) << " py " << st1hit->get_py(0)
                << " pz " << st1hit->get_pz(0) << " px/pz "
                << st1hit->get_px(0) / st1hit->get_pz(0) << std::endl;
      px_st1 = st1hit->get_px(0);
      pz_st1 = st1hit->get_pz(0);
      // z: 616.956

      double tx_st1 = (primary->get_px()) / primary->get_pz();
      x_st1 = gvx + (st1hit->get_z(0) - gvz) *
                        (primary->get_px() / primary->get_pz());
      std::cout << "   calculated x " << x_st1 << " px/pz " << tx_st1
                << std::endl;
    }
    ...
    if (h1hit) {
      std::cout << "h1y t/b x " << h1hit->get_x(0) << " y " << h1hit->get_y(0)
                << " z " << h1hit->get_z(0) << " pz " << h1hit->get_pz(0)
                << std::endl;
    }
    ...
    if (h1xhit) {
      std::cout << "h1x l/r x " << h1xhit->get_x(0) << " y " << h1xhit->get_y(0)
                << " z " << h1xhit->get_z(0) << " pz " << h1xhit->get_pz(0)
                << std::endl;
    }
    ...
    double x_st2 = 0;
    if (st2hit) {
      std::cout << "st2 x " << st2hit->get_x(0) << " y " << st2hit->get_y(0)
                << " z " << st2hit->get_z(0) << std::endl;
      std::cout << "    px " << st2hit->get_px(0) << " py " << st2hit->get_py(0)
                << " pz " << st2hit->get_pz(0) << " px/pz "
                << st2hit->get_px(0) / st2hit->get_pz(0) << std::endl;
      ...
      std::cout << "   kmag kick " << kmag_kick << " center " << kmag_center
                << std::endl;
      if (posel) {
        double tx_st2 = (px_st1 + kmag_kick) / pz_st1;
        double x_st2 = x_st1 + (st2hit->get_z(0) - 1064.26) * tx_st2;
        std::cout << "   calculated x " << x_st2 << " px/pz " << tx_st2
                  << " pxst1 " << px_st1 << std::endl;
      } else {
        double tx_st2 = (px_st1 - kmag_kick) / pz_st1;
        double x_st2 = x_st1 + (st2hit->get_z(0) - 1064.26) * tx_st2;
        std::cout << "   calculated x " << x_st2 << " px/pz " << tx_st2
    ...
    if (h2xhit) {
      std::cout << "h2x l/r x " << h2xhit->get_x(0) << " y " << h2xhit->get_y(0)
                << " z " << h2xhit->get_z(0) << " pz " << h2xhit->get_pz(0)
                << std::endl;
    }
    ...
    if (h2hit) {
      std::cout << "h2y t/b x " << h2hit->get_x(0) << " y " << h2hit->get_y(0)
                << " z " << h2hit->get_z(0) << " pz " << h2hit->get_pz(0)
                << std::endl;
      std::cout << "    px " << h2hit->get_px(0) << " py " << h2hit->get_py(0)
                << " pz " << h2hit->get_pz(0) << " px/pz "
                << h2hit->get_px(0) / h2hit->get_pz(0) << std::endl;

      double tx_h2 = (px_st2) / pz_st2;
      double x_h2 = x_st2 + (h2hit->get_z(0) - z_st2) * tx_h2;
      std::cout << "   calculated x " << x_h2 << " px/pz " << tx_h2
                << std::endl;
    }
    ...
    if (h3hit) {
      std::cout << "h3y t/b x " << h3hit->get_x(0) << " y " << h3hit->get_y(0)
                << " z " << h3hit->get_z(0) << " pz " << h3hit->get_pz(0)
                << std::endl;
    }
    ...
    if (h4hit) {
      std::cout << "h4 t/b x " << h4hit->get_x(0) << " y " << h4hit->get_y(0)
                << " z " << h4hit->get_z(0) << " pz " << h4hit->get_pz(0)
                << std::endl;
      std::cout << "    px " << h4hit->get_px(0) << " py " << h4hit->get_py(0)
                << " pz " << h4hit->get_pz(0) << " px/pz "
                << h4hit->get_px(0) / h4hit->get_pz(0) << std::endl;

      double tx_h4 = (px_st2) / pz_st2;
      double x_h4 = x_st2 + (h4hit->get_z(0) - z_st2) * tx_h4;
      std::cout << "   calculated x " << x_h4 << " px/pz " << tx_h4
                << std::endl;
    }
...
int SimAna::process_event(PHCompositeNode *topNode) {
  ...
  for (int itrk = 0; itrk < n_recTracks; ++itrk) {
    ...
    // std::cout << "******************** (trk->getTargetMom()).Px() " <<
    // (trk->getTargetMom()).Px() << std::endl;
    ...
    track_px_vtx[n_tracks] = (trk->getVertexMom()).X();
    track_py_vtx[n_tracks] = (trk->getVertexMom()).Y();
    track_pz_vtx[n_tracks] = (trk->getVertexMom()).Z();
  ...
  for (int iDimuon = 0; iDimuon < nRecDimuons; ++iDimuon) {
    ...
    dimuon_x_vtx[n_dimuons] = (recDimuon->get_pos()).X();
    dimuon_y_vtx[n_dimuons] = (recDimuon->get_pos()).Y();
    dimuon_z_vtx[n_dimuons] = (recDimuon->get_pos()).Z();
  ...
  // truth dimuon
  // from
  // https://github.com/E1039-Collaboration/e1039-core/blob/master/simulation/g4dst/TruthNodeMaker.cc#L133-L155
  n_truthdimuons = 0;
  for (int idimuon = 0; idimuon < truthDimuonVector->size(); ++idimuon) {
    ...
    truthdimuon_x_vtx[n_truthdimuons] = (truthdimuon->get_pos()).X();
    truthdimuon_y_vtx[n_truthdimuons] = (truthdimuon->get_pos()).Y();
    truthdimuon_z_vtx[n_truthdimuons] = (truthdimuon->get_pos()).Z();
  ...
  for (auto iter = _truth->GetPrimaryParticleRange().first;
    ...
    if (shower != 0) {
      ...
      // std::cout << "shower " << shower->get_x() << " y " << shower->get_y()
      // <<" z " << shower->get_z() << " id " << shower->get_id() << " parent
      // particle " << shower->get_parent_particle_id() << " nhits " <<
      // shower->get_nhits(ECAL_volume) << " edep " <<
      // shower->get_edep(ECAL_volume) << std::endl;
      n_showers++;
    }
    if (n_showers >= 1000)
      break;
  ...
  for (auto iterp = _truth->GetPrimaryParticleRange().first;
    ...
    // std::cout<<"primary " << n_primaries << " id " << primary->get_pid() <<
    // std::endl;
    ...
    if (st3hit) {
      ...
      // std::cout << "st3 x " << st3hit->get_x(0) << " y " << st3hit->get_y(0)
      // <<" z " << st3hit->get_z(0) << std::endl;
    ...
    if (h1hit) {
      ...
      // std::cout << "h1 x " << h1hit->get_x(0) << " y " << h1hit->get_y(0) <<"
      // z " << h1hit->get_z(0) << std::endl;
    ...
    if (h2hit) {
      ...
      // std::cout << "h2 x " << h2hit->get_x(0) << " y " << h2hit->get_y(0) <<"
      // z " << h2hit->get_z(0) << std::endl;
    ...
    if (h3hit) {
      ...
      // std::cout << "h3 x " << h3hit->get_x(0) << " y " << h3hit->get_y(0) <<"
      // z " << h3hit->get_z(0) << std::endl;
    ...
    // if(h4hit_t) {
    // std::cout << "h4y t x " << h4hit_t->get_x(0) << " y " <<
    // h4hit_t->get_y(0) <<" z " << h4hit_t->get_z(0) << std::endl;
    // }
    // PHG4Hit* h4hit_b = FindG4HitAtStation(trkID,g4hc_h4b);
    // if(h4hit_b){
    //  std::cout << "h4y b x " << h4hit_b->get_x(0) << " y " <<
    //  h4hit_b->get_y(0) <<" z " << h4hit_b->get_z(0) << std::endl;
    //}
    PHG4Hit *h4hit = FindG4HitAtStation(trkID, g4hc_h4t);
    if (!h4hit)
      PHG4Hit *h4hit = FindG4HitAtStation(trkID, g4hc_h4b);
    if (h4hit) {
      // std::cout << "h4y x "  << h4hit->get_x(0) << " y " << h4hit->get_y(0)
      // <<" z " << h4hit->get_z(0) << std::endl;
...
int SimAna::GetNodes(PHCompositeNode *topNode) {
  ...
  // trackVector = findNode::getClass<SQTrackVector>(topNode,
  // "SQRecTrackVector"); if(!trackVector) {
  ...
  truthDimuonVector =
      findNode::getClass<SQDimuonVector>(topNode, "SQTruthDimuonVector");
  ...
  if (!g4hc_d1x)
    g4hc_d1x = findNode::getClass<PHG4HitContainer>(topNode, "G4HIT_D0X");
...
void SimAna::MakeTree() {
  ...
  saveTree->Branch("track_x_target", track_x_target,
                   "track_x_target[n_tracks]/F");
  saveTree->Branch("track_y_target", track_y_target,
                   "track_y_target[n_tracks]/F");
  saveTree->Branch("track_z_target", track_z_target,
                   "track_z_target[n_tracks]/F");
  saveTree->Branch("track_px_target", track_px_target,
                   "track_px_target[n_tracks]/F");
  saveTree->Branch("track_py_target", track_py_target,
                   "track_py_target[n_tracks]/F");
  saveTree->Branch("track_pz_target", track_pz_target,
                   "track_pz_target[n_tracks]/F");
  ...
  saveTree->Branch("track_nhits_st1", track_nhits_st1,
                   "track_nhits_st1[n_tracks]/I");
  saveTree->Branch("track_nhits_st2", track_nhits_st2,
                   "track_nhits_st2[n_tracks]/I");
  saveTree->Branch("track_nhits_st3", track_nhits_st3,
                   "track_nhits_st3[n_tracks]/I");
  ...
  saveTree->Branch("truthdimuon_mass", truthdimuon_mass,
                   "dimuon_mass[n_truthdimuons]/F");
  saveTree->Branch("truthdimuon_x_vtx", truthdimuon_x_vtx,
                   "truthdimuon_x_vtx[n_truthdimuons]/F");
  saveTree->Branch("truthdimuon_y_vtx", truthdimuon_y_vtx,
                   "truthdimuon_y_vtx[n_truthdimuons]/F");
  saveTree->Branch("truthdimuon_z_vtx", truthdimuon_z_vtx,
                   "truthdimuon_z_vtx[n_truthdimuons]/F");
  saveTree->Branch("truthdimuon_px", truthdimuon_px,
                   "truthdimuon_px[n_truthdimuons]/F");
  saveTree->Branch("truthdimuon_py", truthdimuon_py,
                   "truthdimuon_py[n_truthdimuons]/F");
  saveTree->Branch("truthdimuon_pz", truthdimuon_pz,
                   "truthdimuon_pz[n_truthdimuons]/F");
  ...
  saveTree->Branch("gx_ecal", gx_ecal,
                   "gx_ecal[n_primaries][100]/F"); // not sure how to make this
                                                   // with the right size
